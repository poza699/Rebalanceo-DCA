...
function roundPreserveSum(values, target) {
  const targetInt = Math.round(target);
  const floors = values.map((v) => Math.floor(v));
  let remainder = targetInt - floors.reduce((s, v) => s + v, 0);
  const order = values
  ...
  // Si remainder fuese negativo (no debería pasar aquí, pero por seguridad)
...
function Dashboard() {
  ...
  const rows = useMemo(() => {
    ...
    const finalRows = items.map((it) => ({ ...it, invest: invest[it.id], status: status[it.id] }));

    // Redondeamos los importes a mostrar para que la suma exacta coincida con la aportación,
    // sin perder la precisión interna usada para decidir el estado de cada activo.
    const roundedAmounts = roundPreserveSum(finalRows.map((r) => r.invest), dcaNum);
    return finalRows.map((r, i) => ({ ...r, invest: roundedAmounts[i] }));
